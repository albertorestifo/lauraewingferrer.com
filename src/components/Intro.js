...
const scaleSprite = (sprite, width) => {
  const ratio = sprite.width / sprite.height
  console.log(ratio)
  sprite.scale.set(width, width * ratio)
}
...
class Intro extends React.PureComponent {
  ...
  handleLoad = () => {
    ...
    const x = scaleLinear().domain([0, 100]).rangeRound([0, width])
    const y = scaleLinear().domain([0, 100]).rangeRound([0, height])
